...
**メイン(デフォルト)のボコーダは Vocos。** HiFi-GAN / MB-iSTFT は同一 mel 契約・同一データで
学習した**比較・評価用**で、デフォルトを置き換えるものではない(既定は `vocos_d256_n512`)。

| ファイル | 種別 | 位置づけ | 構成 | サイズ |
|---|---|---|---|---|
| `vocos_d64_n512_vocoder_slim_fp16.onnx` | Vocos | **メイン** | dim=64, n_fft=512 (tiny, 自前学習) | 0.8 MB |
| `vocos_d128_n512_vocoder_slim_fp16.onnx` | Vocos | **メイン** | dim=128, n_fft=512 (Lite-C) | 1.6 MB |
| `vocos_d256_n512_vocoder_slim_fp16.onnx` | Vocos | **メイン (既定)** | dim=256, n_fft=512 (Lite-A) | 4.3 MB |
| `vocos_d512_n1024_vocoder_slim_fp16.onnx` | Vocos | **メイン** | dim=512, n_fft=1024 (Full) | 29.1 MB |
| `hifigan_ch256_vocoder_slim_fp16.onnx` | HiFi-GAN | 比較用 | init_channels=256 | 3.0 MB |
| `mbistft_n16_vocoder_slim_fp16.onnx` | MB-iSTFT | 比較用 | n_fft=16 | 2.9 MB |
| `mbistft_n64_vocoder_slim_fp16.onnx` | MB-iSTFT | 比較用 | n_fft=64 | 2.8 MB |
